...
import { VaporDEXV2PoolSwapTest } from '../typechain/VaporDEXV2PoolSwapTest'
...
describe('VaporDEXV2Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('VaporDEXV2PoolSwapTest')
            const tester = (await testerFactory.deploy()) as VaporDEXV2PoolSwapTest
          ...
          let tester: VaporDEXV2PoolSwapTest
