...
import { TestVaporDEXV2SwapPay } from '../typechain/TestVaporDEXV2SwapPay'
...
import { TestVaporDEXV2ReentrantCallee } from '../typechain/TestVaporDEXV2ReentrantCallee'
...
describe('VaporDEXV2Pool', () => {
  ...
  let factory: VaporDEXV2Factory
  let pool: MockTimeVaporDEXV2Pool

  let swapTarget: TestVaporDEXV2Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeVaporDEXV2Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestVaporDEXV2ReentrantCallee')
      ).deploy()) as TestVaporDEXV2ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestVaporDEXV2SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestVaporDEXV2SwapPay')
      underpay = (await underpayFactory.deploy()) as TestVaporDEXV2SwapPay
