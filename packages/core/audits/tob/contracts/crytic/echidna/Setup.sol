...
import '../../../../../contracts/VaporDEXV2Pool.sol';
import '../../../../../contracts/VaporDEXV2Factory.sol';
...
contract SetupUniswap {
    ...
    VaporDEXV2Factory factory;

    constructor(TestERC20 _token0, TestERC20 _token1) public {
        factory = new VaporDEXV2Factory();
        token0 = _token0;
        token1 = _token1;
    }

    function createPool(uint24 _fee, uint160 _startPrice) public {
        pool = VaporDEXV2Pool(factory.createPool(address(token0), address(token1), _fee));
...
contract UniswapMinter {
    ...
    function setPool(VaporDEXV2Pool _pool) public {
        pool = _pool;
    }

    function VaporDEXV2MintCallback(uint256 amount0Owed, uint256 amount1Owed, bytes calldata data) external {
...
contract UniswapSwapper {
    ...
    function VaporDEXV2SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external {
