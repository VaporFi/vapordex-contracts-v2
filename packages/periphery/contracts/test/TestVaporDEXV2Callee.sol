...
contract TestVaporDEXV2Callee is IVaporDEXV2SwapCallback {
    ...
    function VaporDEXV2SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external override {
        ...
        if (amount0Delta > 0) {
            IERC20(IVaporDEXV2Pool(msg.sender).token0()).transferFrom(sender, msg.sender, uint256(amount0Delta));
        } else {
            assert(amount1Delta > 0);
            IERC20(IVaporDEXV2Pool(msg.sender).token1()).transferFrom(sender, msg.sender, uint256(amount1Delta));
        }
    }
}
