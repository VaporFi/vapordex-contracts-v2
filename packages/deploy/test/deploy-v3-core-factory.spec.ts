...
import VaporDEXV2Factory from '@uniswap/v3-core/artifacts/contracts/UniswapV3Factory.sol/UniswapV3Factory.json'
...
describe('deploy-v3-core-factory', () => {
  ...
  describe('DEPLOY_V3_CORE_FACTORY', () => {
    it('deploys the v3 core factory contract', async () => {
      ...
      expect(result.message).to.eq('Contract VaporDEXV2Factory deployed')
    })
    ...
    it('does not deploy if already deployed', async () => {
      ...
      expect(result.message).to.eq('Contract VaporDEXV2Factory was already deployed')
    ...
    describe('test contract functions', () => {
      ...
      beforeEach(async () => {
        ...
        v3CoreFactory = new Contract(result.address!, VaporDEXV2Factory.abi, provider)
      })
