...
contract AlgebraQuoterCore {
    ...
    function getPoolGlobalState(
        address pool
    ) internal view returns (GlobalState memory gs) {
        (gs.startPrice, gs.startTick, gs.fee, , , ) = IAlgebraPool(pool)
            .globalState();
    }
    ...
    function getLiquidity(address pool) internal view returns (uint128) {
